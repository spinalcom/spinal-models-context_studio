...
class SpinalGraph extends globalType.Model {
  ...
  async _addExternalIdNodeMappingEntry(_ElementId, _node) {
    ...
    if (typeof _dbid == "number")
      if (_dbid != 0) {
        ...
        _ElementId.unbind(this._addExternalIdNodeMappingEntry.bind(this,
          _ElementId, _node));
      }
  }
  /**
  ...
  async addNodeAsync(_element) {
    ...
    if (_element instanceof _BIMElement2.default) {
      ...
      if (typeof this.externalIdNodeMapping[_element.externalId.get()] !==
        "undefined") {
      ...
      if (typeof this.guidAbstractNodeMapping[_element.id.get()] !==
        "undefined") {
      ...
      if (element instanceof _BIMElement2.default) {
        ...
        if (typeof _dbid == "number")
          if (_dbid != 0) {
            ...
            element.id.bind(this._addExternalIdNodeMappingEntry.bind(
              null, element.id, _node));
          }
  ...
  async addSimpleRelationAsync(_relationType, _node, _element, _isDirected =
    ...
    if (!this.isReserved(_relationType)) {
      ...
      let rel = new _SpinalRelation2.default(_relationType, [_node], [node2],
        _isDirected);
      this.addRelation(rel);
      return rel;
    } else {
      console.log(_relationType + " is reserved by " + this.reservedRelationsNames[
        _relationType]);
    }
  }
  /**
  ...
  addSimpleRelation(relationType, node, element, isDirected = false) {
    if (!this.isReserved(_relationType)) {
      ...
      let rel = new _SpinalRelation2.default(relationType, [node], [node2],
        isDirected);
      this.addRelation(rel);
      return rel;
    } else {
   ...
   * @memberof SpinalGraph
   */
  addSimpleRelationByApp(appName, relationType, node, element, isDirected =
    false) {
  ...
  addRelation(relation, appName) {
    if (this.hasReservationCredentials(relation.type.get(), appName)) {
      ...
      console.log(relation.type.get() + " is reserved by " + this.reservedRelationsNames[
        relation.type.get()]);
    }
  }
  /**
  ...
  hasReservationCredentials(relationType, appName) {
    return !this.isReserved(relationType) || this.isReserved(relationType) &&
  ...
  async getAppsByType(appType) {
    if (typeof this.appsListByType[appType] !== "undefined") return await _Utilities
      .Utilities.promiseLoad(this.appsListByType[appType]);
  }
   ...
   * @memberof SpinalGraph
   */
  async getContext(name, relationsTypesLst, models, Interactions,
    startingNode, relatedGraph = this) {
    if (typeof this.appsList[name] === "undefined") {
      let context = new _SpinalContext2.default(name, relationsTypesLst,
        models, Interactions, startingNode, relatedGraph);
  ...
  async getApp(name, relationsTypesLst, relatedSpinalGraph = this) {
    if (typeof this.appsList[name] === "undefined") {
      let spinalApplication = new _SpinalApplication2.default(name,
        relationsTypesLst, relatedSpinalGraph);
      this.appsList.add_attr({
  ...
  reserveUniqueRelationType(relationType, app) {
    if (typeof this.reservedRelationsNames[relationType] === "undefined" &&
      ...
      return true;
    } else if (typeof this.reservedRelationsNames[relationType] !==
      "undefined") {
      console.error(relationType + " has not been added to app: " + app.name
        .get() + ",Cause : already Reserved by " + this.reservedRelationsNames[
          relationType]);
      return false;
    } else if (typeof this.relationListByType[relationType] !== "undefined") {
      console.error(relationType + " has not been added to app: " + app.name
        .get() + ",Cause : already Used by other Apps");
    }
  }
}
